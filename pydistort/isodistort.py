...
#                      All rights reserved
#
# File coded by:    Parker Hamilton, Tobias Bird
#
...
def _postSubgroupCIF(data, subcif):

    subfname = _uploadCIF(subcif)
    data['input'] = 'uploadsubgroupcif'
    data['filename'] = subfname

    out4 = requests.post(ISO_FORM_SITE, data=data)

    return out4

def _setM4Data(out4, data, basis = [], specify = False):

    #data = {}
    line_iter = out4.iter_lines()
    #for line in line_iter:
...
def getMethod4(parentcif, subcif, outfname, specify = False, basis = [], format = 'structurefile', var_dict = {}):
    """Run ISODISTORT method 4 with subcif as child CIF"""
    parentfname = _uploadCIF(parentcif)
    data = _setData(parentfname, var_dict)
    out = _postSubgroupCIF(data, subcif)
    data = _setM4Data(out, data, basis, specify)
    data = _postDistort(data, format)
    data['zeromodes'] = False
    _postDisplayDistort(data, outfname)

def _postM3CIF(fname,var_dict):
...
def getMethod3(cifname, outfname, var_dict={}, format='topas', selection=1):
    ...
    if format not in formatlist:
        ...
        data = _postM3CIF(parentcif, var_dict)
